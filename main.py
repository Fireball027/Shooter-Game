...
class Player(pygame.sprite.Sprite):
    ...
    def player_turning(self):
        ...
        self.angle = (self.angle) % 360  # If this does not work, add 360 in the brackets
    ...
    def player_input(self):
        ...
        if self.velocity_x != 0 and self.velocity_y != 0:  # Moves diagonally
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.alive:
            self.check_alive()
            if self.get_vector_distance(pygame.math.Vector2(player.base_player_rect.center), pygame.math.Vector2(self.base_zombie_rect.center)) < 100:
...
class UI():
    ...
    def display_enemy_count(self):
        text_1 = font.render(
            f"Enemies: {game_stats['number_of_enemies'][game_stats['current_wave'] - 1] - game_stats['enemies_killed_or_removed']}", True, GREEN)
...
class Item(pygame.sprite.Sprite):
    ...
    def check_player_collision(self):
        if pygame.Rect.colliderect(self.rect, player.base_player_rect):     # Player picks up an item
            self.kill()
...
class GameLevel(pygame.sprite.Group):
    ...
    def create_map(self):
        layouts = {
            "boundary": self.import_csv_layout("data/csvfiles/Map_boundary.csv"),
    ...
    def custom_draw(self):
        self.offset.x = player.rect.centerx - (WIDTH // 2)     # Have to blit the player rect, not the base rect
        ...
        # Drawing the PLAYER'S rectangles for demonstration purposes
        ...
        # Drawing the ZOMBIE'S rectangles for demonstration purposes
...
while True:
    ...
    if game_active:
        ...
        if game_stats['enemies_killed_or_removed'] == game_stats["number_of_enemies"][
            game_stats["current_wave"] - 1]:     # Level over
            display_countdown_time = True
            ready_to_spawn = True
            level_over_time = pygame.time.get_ticks()
            game_stats["enemies_killed_or_removed"] = 0
            game_stats["current_wave"] += 1
            if game_stats["current_wave"] == len(game_stats["number_of_enemies"]) + 1:    # Beat the game
                beat_game = True
                end_game()
            else:
                beat_game = False
